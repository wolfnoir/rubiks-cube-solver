...
class A_Star_Cross(CrossSolver):
...
class A_Star_F2LPairSolver(object):
    """
    ...
    def combining_successors(state, last_action=()):
        ...
        for act in (U_turns + R_turns + F_turns):
            new = (corner, edge)
            for q in act:
                new = A_Star_F2LPairSolver._rotate(new, q)
    ...
    def solve(self):
        ...
        for i in range(4):
            for slot in ["FR", "RB", "BL", "LF"]:
                solver = A_Star_F2LPairSolver(self.cube, slot)
